...
function App() {
  ...
  const toggleComplete = async (task) => {
    if (secondsSince(task.createdAt) > TIMEOUT_SECONDS) {
      Swal.fire({ icon: 'error', title: 'Timed Out', text: "Can't complete now!", confirmButtonText: 'OK' });
      return;
    }
  ...
  const editTask = async (task) => {
    ...
    if (newText && newText.trim()) {
      await axios.put(`${API_URL}/tasks/${task._id}`, {
        text: newText.trim(),
        completed: false,
        createdAt: new Date().toISOString(), // Reset time
      });
        // 3. Refresh task list here:
        await fetchTasks();
      Swal.fire({
        icon: 'success',
        title: 'Updated!',
  ...
  const viewTask = (task) => {
    Swal.fire({
      ...
      html: `
        ...
        <p><strong>Status:</strong> ${task.completed ? '✅ Completed'
          : secondsSince(task.createdAt) > TIMEOUT_SECONDS ? '⛔ Timed Out'
            : '⏳ Pending'
        }</p>
        <p><strong>Created At:</strong> ${new Date(task.createdAt).toLocaleTimeString()}</p>
      `,
      confirmButtonText: 'OK',
    });
  };
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col items-center p-8">
      <div className="w-full max-w-md bg-white shadow-lg rounded-lg p-6">
        <h1 className="text-2xl font-bold text-center mb-4">⏰ Smart To-Do</h1>
        <form
          onSubmit={(e) => {
            e.preventDefault();
            addTask();
        ...
        <ul className="space-y-3">
          {tasks.map((task) => {
            const secs = secondsSince(task.createdAt);
            const timedOut = secs > TIMEOUT_SECONDS;
            return (
              <li key={task._id} className="flex flex-col sm:flex-row sm:items-center justify-between bg-gray-50 p-4 rounded shadow space-y-2 sm:space-y-0">
                <div>
                  <span
                    ...
                    className={`cursor-pointer ${task.completed ? 'line-through text-green-600'
                      : timedOut ? 'text-red-600 italic'
                        : 'text-gray-800'
                      }`}
                  >
                    {task.text}
                  ...
                  <span className="ml-2 text-sm">
                    {task.completed ? 'Complete' : timedOut ? 'Timed Out' : `${TIMEOUT_SECONDS - secs}s left`}
                  </span>
                </div>
